...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // === Floating language button (same as Home / OurStory / OurTeam) ===
    ...
    if (nav && ul && langLi && langBtn) {
        // Placeholder to restore button back on desktop
        ...
        // Create floating container for mobile
    ...
    // === Particle initialization (untouched) ===
    if (window.location.pathname.includes('Sponsors.html')) {
        const debouncedParticleUpdate = debounce(updateSponsorParticles, 150);
        updateSponsorParticles();
...
function createOrUpdateParticles(particleCount) {
    ...
    const baseSpeed = 40; // pixels per second (slower)
...
function updateSponsorParticles() {
    const particlesContainer = document.getElementById('particles');
    if (!particlesContainer) return;
...
// === Sponsor title image fallback (unchanged) ===
document.addEventListener('DOMContentLoaded', function() {
    const titleImage = document.getElementById('sponsor-title-image');
    const titleText = document.getElementById('sponsor-title-text');
    ...
    imageTester.src = titleImage.src;
});
